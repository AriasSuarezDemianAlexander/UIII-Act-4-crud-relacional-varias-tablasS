...
$sentencia = $base_de_datos->query("SELECT tbl_pedidos.total, tbl_pedidos.fecha, tbl_pedidos.id, GROUP_CONCAT(	tbl_productos.codigo, '..', tbl_productos.nombrep, '..',  tbl_productos.descr, '..', tbl_productos.preciou, '..',  tbl_productos.stockd, '..', tbl_productos.unidadm, '..',  tbl_productos.fechai, '..', tbl_productos.marca, '..', tbl_productosvendidos.cantidad SEPARATOR '__') AS tbl_productos FROM tbl_pedidos INNER JOIN tbl_productosvendidos ON tbl_productosvendidos.id_venta = tbl_pedidos.id INNER JOIN tbl_productos ON tbl_productos.id = tbl_productosvendidos.id_producto GROUP BY tbl_pedidos.id ORDER BY tbl_pedidos.id;");
